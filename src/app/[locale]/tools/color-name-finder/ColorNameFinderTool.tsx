...
export default function ColorNameFinderTool() {
  ...
  const [hexInput, setHexInput] = useState("#487877");
  const [colorPickerValue, setColorPickerValue] = useState("#487877");
  ...
  const matches = useMemo((): ColorMatch[] => {
    ...
    const withDist = CSS_NAMED_COLORS.map((color) => ({
      color,
      distance: colorDistance(rgb.r, rgb.g, rgb.b, color.r, color.g, color.b),
    }));
  ...
  const inputRgb = useMemo(() => hexToRgb(normalizedHex), [normalizedHex]);
  const isValidHex = inputRgb !== null;
  ...
  const handleHexInputChange = (value: string) => {
    ...
    if (rgb) {
      ...
      if (clean.length === 6 || clean.length === 3) {
        setColorPickerValue(value.startsWith("#") ? value : `#${value}`);
      }
    }
  };

  return (
    <ToolLayout toolSlug="color-name-finder">
      <div className="space-y-6">
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <div className="flex gap-3 items-center">
            <input
              ...
              className="w-14 h-14 rounded-xl border border-border cursor-pointer flex-shrink-0"
            />
        ...
        {isValidHex && (
          <div className="flex items-center gap-4">
            <div
              className="w-20 h-20 rounded-xl border border-border flex-shrink-0"
              style={{ backgroundColor: normalizedHex }}
            />
            <div>
              <p className="text-sm font-medium text-foreground">Your Color</p>
              <p className="text-lg font-mono font-bold text-foreground">{normalizedHex.toUpperCase()}</p>
              {inputRgb && (
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <div className="flex items-center gap-5">
              <div className="flex gap-1">
                <div
                  className="w-20 h-20 rounded-l-xl border border-border"
                  style={{ backgroundColor: normalizedHex }}
                />
                <div
                  className="w-20 h-20 rounded-r-xl border border-border"
                  style={{ backgroundColor: matches[0].color.hex }}
                />
              </div>
              <div>
                <p className="text-xl font-bold text-foreground">{matches[0].color.name}</p>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-sm font-mono text-muted-foreground">{matches[0].color.hex}</span>
                  <CopyButton text={matches[0].color.name.toLowerCase()} />
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {t("distance")}: {matches[0].distance.toFixed(1)}
                  {matches[0].distance === 0 && " (exact match!)"}
                </p>
              </div>
            </div>
          </div>
              ...
              {matches.map((match, i) => (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg transition-colors ${
                    i === 0 ? "bg-teal/5 border border-teal/20" : "bg-surface"
                  }`}
                >
                  <span className="text-sm font-mono text-muted-foreground w-5 text-center flex-shrink-0">
                    {i + 1}
                  </span>
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground">{match.color.name}</p>
                    <p className="text-xs font-mono text-muted-foreground">{match.color.hex}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-xs text-muted-foreground font-mono">
                      {t("distance")}: {match.distance.toFixed(1)}
                    </p>
                  </div>
                  <CopyButton text={match.color.hex} />
                </div>
              ))}
            </div>
          </div>
        ...
        {!isValidHex && hexInput.length > 0 && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-sm text-red-600">
            Please enter a valid hex color (e.g. #FF5733 or #F00)
          </div>
        )}
      </div>
    </ToolLayout>
